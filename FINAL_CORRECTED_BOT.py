#!/usr/bin/env python3
"""
بوت مراقبة السيولة المتكامل - النسخة النهائية المصححة
...
class LiquidityBot:
    ...
    def is_halal_stock(self, symbol: str, stock_data: StockData) -> bool:
        ...
        if financial_data:
            # الشرط 1: نسبة ديون قليلة (للشركات الشرعية)
            debt_ratio = financial_data.get('debt', 0)
            ...
            if net_margin < MIN_NET_PROFIT_MARGIN:
                return False
            
            # الشرط 3: نسبة السيولة أكبر من 1
            current_ratio = financial_data.get('current_ratio', 0)
            if current_ratio > 0 and current_ratio < MIN_CURRENT_RATIO:
                return False
        ...
        try:
            ...
            if self.check_liquidity_entry(symbol, stock_data) and self.can_send_alert(symbol, "entry"):
                ...
                message = self.format_alert_message(symbol, stock_data, "entry",
                                                  fair_value, rsi, support, resistance)
                self.send_telegram_message(message, CHANNEL_GENERAL)
